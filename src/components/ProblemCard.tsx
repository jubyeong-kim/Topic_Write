...
export default function ProblemCard({ problemType, problemCount }: ProblemCardProps) {
  const typeKey = problemType.typeNumber <= 52 ? 'fill_blank' : problemType.type === 'data_description' ? 'data_description' : 'essay';

  return (
    <Link href={`/problem?type=${problemType.type}&num=${problemType.typeNumber}`}>
      <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow p-6 cursor-pointer border border-gray-100">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{problemCount}개 문제 | {problemType.points}점</p>
          </div>
        </div>
        <p className="text-gray-600 text-sm leading-relaxed">
          {problemType.description}
        </p>
        {problemType.wordLimit && (
          <p className="text-xs text-orange-500 mt-2 font-medium">
            분량: {problemType.wordLimit}
          </p>
        )}
