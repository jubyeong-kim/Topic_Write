...
- **사진 OCR 입력은 조건부 가능** — 단독은 완전일치 32% 로 부족하나,
  **앙상블(CLOVA+surya) → 불일치 탐지 → LLM 객관식 판별**로 자동화율 77%
  ([권장 설계](../topik-ocr-poc/README.md#-권장-설계))
- **비전 LLM 은 전사에 쓰지 않는다** — CER 0.010 으로 최고지만 `이였다`→`이었다` 처럼
  학습자 오류를 고쳐 읽는다. **객관식 판별에만** 쓰면 보존된다(92%)
