...
| **사진 OCR** | ⚠️ **조건부 가능** | 단독으로는 완전일치 32% 로 부족하나, **앙상블 + 판별 설계**로 자동화율 77% 도달. 아래 참조 |
| **태블릿·터치 필기** | 미검증, **유망** | 획 순서·방향 정보가 있어 사진 OCR 보다 인식률이 크게 높다 |

#### 사진 OCR — "기각"에서 "조건부 가능"으로 정정

초기 검증에서 완전일치 32% 를 이유로 기각했으나, 이후 실험으로 **설계를 갖추면 쓸 수 있음**이 확인됐다.

```
답안 이미지 → CLOVA + surya 병렬 → 어절 대조
                                  ├ 합의 60% (정확도 96%) → 채택
                                  ├ 불일치 중 37% → LLM 객관식 판별 (92%)
                                  └ 나머지 → 사용자 확인
```

**자동화율 약 77%**, 나머지는 사용자가 표시된 곳만 확인한다.

⚠️ **주의 — 비전 LLM 단독은 쓰지 않는다.** 정확도는 최고(CER 0.010)이나
`이였다`→`이었다` 처럼 **학습자 오류를 고쳐 읽어** 채점 대상을 지운다.
LLM 은 **전사가 아니라 객관식 판별**에만 쓴다(그때는 오류가 보존됨).

상세: [OCR PoC 권장 설계](../topik-ocr-poc/README.md#-권장-설계)
